        ...
        <div id="content">
            <div id="main-content" class="container" style="margin-top: 100px">
                ...
                <div class="form-section mb-4">
                    ...
                    <form action="../routes/member.php" method="post">
                        <div class="form-group">
                            <label for="namaOutlet">Nama</label>
                            <input type="text" class="form-control" id="namaOutlet" name="nama" value="<?= isset($edit['nama']) ? htmlspecialchars($edit['nama']) : '' ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="jenisPaket">Jenis Kelamin</label>
                            <select class="form-control" id="jenisPaket" name="jenkel" required>
                                <option value="">Pilih Jenis Kelamin</option>
                                <option value="Laki-laki" <?= isset($edit['jenkel']) && $edit['jenkel'] === 'Laki-laki' ? 'selected' : '' ?>>Laki-laki</option>
                                <option value="Perempuan" <?= isset($edit['jenkel']) && $edit['jenkel'] === 'Perempuan' ? 'selected' : '' ?>>Perempuan</option>
                            </select>
                        </div>
                        <div class="form-group">
                    ...
                    <table class="table table-bordered table-striped mt-4">
                        ...
                        ?>
                            <tr>
                                <td><?=$no++?></td>
                                <td><?=$members['nama']?></td>
                                <td><?=$members['jenkel']?></td>
                                <td><?=$members['alamat']?></td>
                                <td><?=$members['telepon']?></td>
                                <td>
                                    <a class="btn btn-warning btn-sm" href="member.php?id=<?=md5($members['id'])?>">Edit</a>
                                    <a href="../routes/member.php?id=<?=md5($members['id'])?>&&action=destroy" class="btn btn-danger btn-sm">Hapus</a>
                                </td>
                            </tr>
                        <?php
                        }
